...
const steps = [
  ...
  {
    title: '等待当前对象',
    detail: '管理员切换对象后，评分端会自动刷新；未显示时可点击页面右上角刷新。',
  },
  {
    title: '逐项完成评分',
  ...
  {
    title: '确认并提交',
    detail: '全部评分确认无误后提交。同一设备对同一对象只能提交一次有效评分。',
  },
]

const systemRules = [
  '请使用本人手机完成评分，不要借用他人设备或代他人评分。',
  '本次活动的参评对象不得进入评分端，不得参与任何对象的评分。',
  ...
  '如出现填错姓名、无法进入、对象未刷新等情况，请立即联系现场管理员。',
]

const scoreRules = [
  '请依据参评对象现场表现或材料表现独立评分，不要相互询问或讨论分数。',
  '评分提交前请认真核对，提交后等待管理员切换下一位对象。',
  '对象切换后，上一位对象的评分会锁定，评分端无法自行回改。',
        ...
        <header className="mb-8 flex items-start justify-between gap-10">
          <div>
            <div className="mb-4 inline-flex items-center gap-3 rounded-full border border-teal-400/30 bg-teal-400/10 px-4 py-2 text-teal-200">
              <Smartphone size={22} />
              <span className="text-lg font-semibold">活动评分系统使用说明</span>
            </div>
            ...
            <p className="mt-5 text-2xl leading-relaxed text-slate-300">
              进入评分端后，按身份输入口令，填写本人真实姓名，并根据当前对象表现独立评分。
            </p>
          </div>
            ...
            <div className="rounded-md border border-amber-300/30 bg-amber-300/10 p-6">
              ...
              <div className="flex items-start gap-4 rounded-md bg-slate-950/60 p-5">
                ...
                <p className="text-2xl font-semibold leading-relaxed text-white">
                  必须填写 2 或 3 个汉字的本人真实姓名。参评对象不得参与评分；使用网名、昵称、数字、字母、乱填姓名或冒用他人姓名，可能被管理员封禁设备，相关评分将作废。
                </p>
              </div>
            </div>
